...
void ConvolutionalLayer::computeNaive(const LayerData& dataIn) const 
{
    ...
    for(int fill = 0; fill < numFilters; fill++)
    {
        ...
        //uint32_t curBias = biasData.get<fp32>(fill);
                ...
                for(int dep = 0; dep < depth; dep++)
                {
                    for(int fill_h = 0; fill_h < filterHeight; fill_h++)
                    {
                        for(int fill_w = 0; fill_w < filterWidth; fill_w++)       
                        {
                            ...
                            int8_t quantized_inputValue= round(scaleValueInputs*inputValue) + zero_points;
                            //float filterValue = weightData.get<fp32>(fInd);
                            int8_t quantized_filterValue = weightData.get<int8_t>(fInd);
...
//     for(int fill = 0; fill < numFilters; fill++)
//     {
//         uint8_t curBias = biasData.get<fp32>(fill);
//         for(int out_h = 0; out_h < outputHeight; out_h++)
//         {
//             for(int out_w = 0; out_w < outputWidth; out_w++)
//             {
        ...
        {    
            for(int fill = 0; fill < numFilters; fill++)
            {
                for(int fill_h = 0; fill_h < filterHeight; fill_h++)
                {
                    for(int fill_w = 0; fill_w < filterWidth; fill_w++)       
                    {
                        for(int dep = 0; dep < depth; dep++)
                        {
                            ...
                            int16_t DataValue= ((int8_t)filterValue << 8) | (int8_t)quantized_value;
                            Xil_Out32(XPAR_AXI_FIFO_0_BASEADDR + XLLF_TDFD_OFFSET, (int32_t)DataValue);
                ...
                while (true) {
                    ...
                    int32_t read_len = Xil_In32(XPAR_AXI_FIFO_0_BASEADDR + XLLF_RLF_OFFSET);
